...
std::vector<std::vector<double> > Matrix::getPMW(std::vector <std::string> DNAsequences)
{
    ...
    for (size_t i(0); i < getLenghtMotif(); ++i) //on boucle selon la i-eme valeur du string
    {
        ...
        std::vector <double> all_proba {proba_A,proba_T,proba_G,proba_C};
...
double Matrix::score(Matrix m, std::string DNAsequence, double limit)
{
    double score(0);
